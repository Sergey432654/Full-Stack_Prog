...
function App() {
  ...
  const[card, setShowCard] = useState<boolean>(false);
  const {register , handleSubmit, setError, formState: { errors }} = useForm<FormFields>();
  const [savedCardData, setSavedCardData] = useState<FormFields | null>(null);
...
const Egorb = () => {
  ...
  setError("number", { type: "custom", message: "custom message" })
}
React.useEffect(() => {
  setError("name", {
    type: "manual",
  ...
  return (
   ...
   <div className="form__group">
                <input
                  ...
                  {...register("number", { pattern: /^(\d{4}\s){3}\d{4}$/ })}
                />
   ...
   />
     ...
     <div className="flip-card">
      <div className="flip-card-inner">
         <div className="flip-card-front">
            ...
            <p className="number">{savedCardData?.number}</p>
            <p className="valid_thru">VALID THRU</p>
            <p className="date_8264">{savedCardData?.date}</p>
            <p className="name">{savedCardData?.name}</p>
        </div>
    ...
    <>
      ...
      <button onClick={paySistem}>Pay</button>
